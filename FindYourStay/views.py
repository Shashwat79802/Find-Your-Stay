...
def output_file_name_generator(city, state):
    ...
    return dir_path + "/" + city + '_' + state + '.csv'
...
def oyo_scrapper(city, state):
    ...
    with open(dir_path+'/oyo.csv', mode='w', newline='', encoding='utf-8') as file:
...
def yatra_scrapper(city):
    ...
    with open(dir_path+'/yatra.csv', mode='w', newline='', encoding='utf-8') as file:
        writer = csv.writer(file)
        writer.writerow(['Hotel Name', 'Price', 'Image', 'Address', 'Float Rating', 'Rating', 'Hotel URL'])
...
def scrapper(city, state):
    ...
    yatra_scrapper(city)
    data_cleaner(dir_path+'/oyo.csv')
    data_cleaner(dir_path+'/yatra.csv')

    output_file = output_file_name_generator(city, state)

    combine_csv(dir_path+'/oyo.csv', dir_path+'/yatra.csv', output_file)
